...
#include "ov2180_mipi_raw_Sensor.h"
#ifdef CONFIG_MTK_ENG_BUILD
#define LOG_INF(format, args...)    \
	printk(" ov2180_mipi_raw_Sensor [%s] " format, __func__, ##args)
#else
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = 0x2682,		//OV2180MIPI_SENSOR_ID,  /*sensor_id = 0x2180*/ //record sensor id defined in Kd_imgsensor.h
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id() + 2;
			if ((*sensor_id == imgsensor_info.sensor_id) && (0x7 == vendor_id)) {
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			} else {
			pr_err("ov2180_ofilm check id fail i2c write id: 0x%x, sensor id: 0x%x vendor_id: 0x%x\n",
